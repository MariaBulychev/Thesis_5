...
test_subset = Subset(test_dataset, range(4))
...
for batch_idx, (images, true_labels) in enumerate(test_loader):
    ...
    # Analyze and print the most important concepts and the class labels for each image
    for i, (true_label, predicted_label) in enumerate(zip(true_class_labels, predicted_class_labels)):
        ...
        # Get the distances for the current image and sort them by absolute value
        image_distances = distances[i]
        sorted_indices = torch.argsort(image_distances.abs(), descending=False)
        
        # Print the top N concept influences
        N = 5  # Change N to however many top concepts you want to print
        print("Most influential concepts:")
        for j in range(N):
            concept_index = sorted_indices[j]
            concept_name = sorted_concept_list[concept_index]
