...
def get_projections_ft(args, backbone, posthoc_layer, loader):
    all_projs, all_embs, all_lbls = None, None, None
    for batch in tqdm(loader):
        batch_X, batch_Y = unpack_batch(batch)
        batch_X = batch_X.to(args.device)
        #batch_X = preprocess(batch_X) ############################################# new because we load the images without preprocessing
        if "clip" in args.backbone_name:
            embeddings = backbone.encode_image(batch_X).detach().float()
        else:
            embeddings = backbone(batch_X).detach()
...
def load_or_compute_projections_ft(args, backbone, posthoc_layer, train_loader, test_loader):
    # Get a clean conceptbank string
    # e.g. if the path is /../../cub_resnet-cub_0.1_100.pkl, then the conceptbank string is resnet-cub_0.1_100
    conceptbank_source = args.concept_bank.split("/")[-1].split(".")[0] 
    ...
    ################################# changed the name to _ft to make sure that the projections are computed again for the finetuned clip model 
    train_file = f"train-embs_ft{args.dataset}__{args.backbone_name}__{conceptbank_source}.npy"
    test_file = f"test-embs_ft{args.dataset}__{args.backbone_name}__{conceptbank_source}.npy"
    train_proj_file = f"train-proj_ft{args.dataset}__{args.backbone_name}__{conceptbank_source}.npy"
    test_proj_file = f"test-proj_ft{args.dataset}__{args.backbone_name}__{conceptbank_source}.npy"
    train_lbls_file = f"train-lbls_ft{args.dataset}__{args.backbone_name}__{conceptbank_source}_lbls.npy"
    test_lbls_file = f"test-lbls_ft{args.dataset}__{args.backbone_name}__{conceptbank_source}_lbls.npy"
    ...
    if os.path.exists(train_proj_file):
        ...
        print("Projections do not exist.")
        train_embs, train_projs, train_lbls = get_projections_ft(args, backbone, posthoc_layer, train_loader)
        test_embs, test_projs, test_lbls = get_projections_ft(args, backbone, posthoc_layer, test_loader)
