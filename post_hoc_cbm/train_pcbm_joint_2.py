...
def train_epoch(pcbm, clip_model, train_loader, criterion, clip_optimizer, preprocess, device):
    ...
    for inputs, labels in train_loader:
        ...
        if len(unique_classes) != 10: # num_classes
            # Handle the missing classes
            # Option 1: Skip the batch
        ...
        # ensuring classifier size 
        #dummy_features = np.zeros((10, 170))  # 170 features, one for each class
        #dummy_labels = np.arange(10)  # Labels for each class
        #classifier.fit(dummy_features, dummy_labels)
        # fit classifier to real data
        classifier.fit(proj.cpu(), labels.cpu()) ### müsste nicht features fitten sondern die abstände
