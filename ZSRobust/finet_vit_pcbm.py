...
def parse_option():
    ...
    parser.add_argument('--model_dir', type=str, default='./save/models/clip',
                        help='path to save models')
    parser.add_argument('--image_dir', type=str, default='./save/images/clip',
...
mu = torch.tensor(CIFAR100_MEAN).view(3, 1, 1).to(device)#.cuda()
std = torch.tensor(CIFAR100_STD).view(3, 1, 1).to(device)#.cuda()
...
def main():
    ...
    #model, preprocess = clip.load('RN50', device, jit=False, prompt_len=add_prompt_len)
    ...
    #prompter = torch.nn.DataParallel(prompter).to(device)#.cuda()
    #add_prompter = torch.nn.DataParallel(add_prompter).to(device)#.cuda()
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.mix_alpha > 0:
                ...
                model.visual.load_state_dict(theta)
...
def attack_CW(prompter, model, model_text, model_image, add_prompter, criterion, X, target, text_tokens, alpha,
    ...
    delta = torch.zeros_like(X).to(device)#.cuda()
    if norm == "l_inf":
    ...
    for _ in range(attack_iters):
        ...
        label_mask = label_mask.to(device)#.cuda()
