...
class ModelWrapper(nn.Module):
    ...
    def forward(self, images, prompt):
        ...
        features = self.clip_model.encode_image(images, prompt)
        #features = self.clip_model(images)
        out = self.pcbm(features.float().to(device), return_dist = False)
        return out
    
    def trainable_parameters(self):
        # Get trainable parameters from both clip_model.visual and pcbm
        clip_params = list(self.clip_model.visual.parameters())
        pcbm_params = list(self.pcbm.trainable_params())
        
        # Concatenate and return the trainable parameters
        return clip_params + pcbm_params
...
def main():
    ...
    clip_model, _ = clip.load('RN50', device, jit=False)#, prompt_len=add_prompt_len)
    #model_text, 
    model_image = None  #, None
    ...
    #pcbm = torch.nn.DataParallel(pcbm)  # .to(device)
    pcbm.eval()

    resolution = 224
    wrapped_model = ModelWrapper(pcbm, clip_model, resolution)
    convert_models_to_fp32(wrapped_model)
    #wrapped_model = torch.nn.DataParallel(wrapped_model)  # .to(device)
    wrapped_model.eval()
    ...
    if args.last_num_ft == -1:
        optimizer = torch.optim.SGD(wrapped_model.trainable_parameters(),#wrapped_model.clip_model.visual.parameters(),
                                    lr=args.learning_rate,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
    else:
        optimizer = torch.optim.SGD(list(wrapped_model.trainable_parameters())[-args.last_num_ft:],#list(wrapped_model.clip_model.visual.parameters())[-args.last_num_ft:],
                                    lr=args.learning_rate,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
    ...
    if args.evaluate:
        acc1_mean = validate(val_loader_list, val_dataset_name, texts_list, wrapped_model, model_text, model_image,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, texts_train, wrapped_model, model_text, model_image, prompter, add_prompter, optimizer, scheduler,
        ...
        save_checkpoint({
            ...
            'vision_encoder_state_dict': wrapped_model.clip_model.visual.state_dict(),
            'best_acc1': best_acc1,
...
def attack_pgd_noprompt(prompter, model, model_text, model_image, criterion, X, target, text_tokens, alpha,
    ...
    for _ in range(attack_iters):
        ...
        _images = clip_img_preprocessing(X + delta)
        output, _ = multiGPU_CLIP(model_image, model_text, model, _images, text_tokens, None) #takes only logits per image
...
def multiGPU_CLIP(model_image, model_text, model, images, text_tokens, prompt_token=None):
    ...
    img_embed = model(images) #, text_tokens, prompt_token)
    scale_text_embed = None
...
def train(train_loader, texts, model, model_text, model_image, prompter, add_prompter,
    ...
    for i, (images, target) in enumerate(tqdm(train_loader)):
        ...
        model.clip_model.logit_scale.data = torch.clamp(model.clip_model.logit_scale.data, 0, 4.6052)
        ...
        if i % args.save_freq == 0:
            save_checkpoint({
                ...
                'state_dict': prompter.state_dict(),
                'add_prompter': add_prompter.state_dict(),
                'vision_encoder_state_dict': model.clip_model.visual.state_dict(),
                'best_acc1': best_acc1,
                'optimizer': optimizer.state_dict(),
                'wrapped_model': model.state_dict(),
            }, args)
