...
def main():
    ...
    if args.last_num_ft == -1:
        optimizer = torch.optim.SGD(wrapped_model.parameters(),#wrapped_model.clip_model.visual.parameters(),
                                    lr=args.learning_rate,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
    else:
        optimizer = torch.optim.SGD(list(wrapped_model.parameters())[-args.last_num_ft:],#list(wrapped_model.clip_model.visual.parameters())[-args.last_num_ft:],
                                    lr=args.learning_rate,
                                    momentum=args.momentum,
                                    weight_decay=args.weight_decay)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.mix_alpha > 0:
                ...
                wrapped_model.clip_model.visual.load_state_dict(theta)

            else:

                wrapped_model.clip_model.visual.load_state_dict(checkpoint['vision_encoder_state_dict'])
